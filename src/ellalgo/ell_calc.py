"""
Ellipsoid calculation routines for the ellipsoid method.

The `EllCalc` class provides high-level dispatch logic for determining how to
update an ellipsoid based on different types of cutting planes. It selects among
single deep cuts, parallel cuts, and central cuts, delegating the low-level
mathematical computations to `EllCalcCore`.

Cut types supported:
    - Deep cut (bias cut): A hyperplane that does not pass through the center.
    - Central cut: A hyperplane passing through the ellipsoid center.
    - Parallel cut: Two parallel hyperplanes cutting the ellipsoid.
    - Quantized variants: Versions optimized for discrete optimization stability.
"""
