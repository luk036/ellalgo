...
def ell_to_patch(ell: Ell, **kwargs):
    ...
    return MplEllipse(
        xy=ell._xc, width=width, height=height, angle=angle, **kwargs
    )
...
for _ in range(10):
    snapshots.append((
        ell._xc.copy(), ell._mq.copy(), ell._kappa
    ))
...
patch = ell_to_patch(
    type('_', (), {'_xc': snapshots[0][0], '_mq': snapshots[0][1],
                   '_kappa': snapshots[0][2]})(),
    fill=False, edgecolor="#1565C0", linewidth=2, label="Initial"
)
ax.add_patch(patch)
...
final = snapshots[-1]
patch = ell_to_patch(
    type('_', (), {'_xc': final[0], '_mq': final[1],
                   '_kappa': final[2]})(),
    fill=False, edgecolor="#E65100", linewidth=2, label=f"Iter {len(snapshots)}"
)
ax.add_patch(patch)
