...
def valid_ellipsoid_strategy(draw):
    ...
    xc = draw(st.lists(st.floats(min_value=-10, max_value=10), 
...
def valid_cut_strategy(draw, ndim):
    ...
    grad = draw(st.lists(st.floats(min_value=-10, max_value=10), 
                        min_size=ndim, max_size=ndim))
    grad = np.array(grad)
...
class TestEllipsoidProperties:
    ...
    def test_parallel_cut_properties(self, ell_params, cut):
        ...
        assert status_central in [CutStatus.Success, CutStatus.NoSoln, CutStatus.NoEffect]
    ...
    @given(st.floats(min_value=0.001, max_value=10), 
           st.lists(st.floats(min_value=0.1, max_value=5), 
    ...
    @given(st.lists(st.floats(min_value=0, max_value=0), 
